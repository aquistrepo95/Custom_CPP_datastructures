...
constexpr T* allocator<T> :: allocate(std::size_t size) {
    ...
    // allocate memory through global new operator and assign it to a void pointer since global new only assigns raw bytes
...
// allocator comparison for containers, verify if they share the same memory pool
